...
// count events in the LAST 60_000 ms, within 1% relative error, in a fixed ~53 KB pool
// (default maxCount 2^32; pass { maxCount } to size the pool for your window population)
...
- **ExponentialHistogram** -- sliding-window count / sum ("how many / how much in the last `W`?") in a fixed pool of `(timestamp, size)` buckets, with a *hard* windowed COUNT relative-error bound `<= epsilon` (the SUM error is bounded absolutely by half the oldest straddling bucket's value-mass -- relative `<= epsilon` only for count or near-constant values). A `maxCount` option (default 2^32) sizes the pool to the window population. The reference member; DGIM (the 0/1 count stream) is its `value = 1` special case.
...
The same invariant fixes the memory: the pool is sized from `maxCount` (the window population it is guaranteed to hold, default 2^32), not from `W`. The number of levels is `levels = max(2, ceil(log2(maxCount/(k+1))) + 2)`, each level holds at most `k + 1` buckets, so the pool is a fixed
`CAP = (k+1) * levels + 2` buckets -- preallocated at construction, never grown. At `epsilon = 0.01` with the default `maxCount = 2^32` that is 1510 buckets (~53 KB), independent of `W`, versus an exact ring of 65536 timestamps (512 KB at `W = 65536`).

A time-based window holds as many items as the arrival rate allows, so the population bound must be DECLARED -- a fixed pool cannot grow. `maxCount` is a guaranteed FLOOR: the exact ceiling is `k * (2^levels - 1)` elements (>= `maxCount`, ~3-6x it; `epsilon = 0.01, maxCount = 5000` holds 26,061), and the add past it (its merge cascade would pass the top level) throws a tagged `[lite-adaptive]` RangeError (a byte-identical no-op, pre-checked with a read-only expiry scan). Size `maxCount` to your peak window population:

| epsilon (k) | maxCount 5e3 (5 s @ 1 kHz) | maxCount 5e5 (5 s @ 100 kHz) | maxCount 2^32 (default) |
|-------------|---------------------------|------------------------------|-------------------------|
| 0.01 (k 51) | 16.5 KB | 29.3 KB | 53.1 KB |
| 0.05 (k 11) | 4.7 KB  | 7.7 KB  | 13.1 KB |
| 0.1 (k 6)   | 3.0 KB  | 4.7 KB  | 7.9 KB  |

The pool is allocated before the mode locks, so a COUNT-mode instance ALSO gets the default 2^32 sizing (e.g. `EH(1000, 0.01)` is ~53 KB, not ~13 KB). Pass `maxCount: W` to keep the pre-1.7.0 W-sized pool.

The measured windowed COUNT relative error tracks `1/(2k)` and stays under `epsilon` across the whole `W in {64, 1000, 65536} x epsilon in {0.5, 0.1, 0.01}` sweep, plus a dense 10 kHz / 70 s stream that never goes NaN -- see [Testing](#testing). The SUM error obeys `|sum_est - sum_true| <= size(oldest straddling bucket) / 2`, which is relative `<= epsilon` only for count or near-constant values (a heavy-tailed or spiky value stream can exceed it). For an idle stream, `advance(now)` (and the zero-box `advanceFrom(buf, i)`) expires the window edge with no value added, so `count()` an hour later reflects the empty window instead of the last burst (R11 idle-slide, 0 B/op).
</details>
...
`sigmaHat^2` is the total-window variance and `R` is the current window's range (`max - min` over the live buckets, excluding the oldest -- since 1.7.0 F18; pre-1.7.0 this was a running global range that never shrank and could go deaf after a level shift). This is the **ADWIN2 variance-aware bound** (Bifet-Gavalda, SDM 2007): the variance term dominates on low-variance streams, so it detects small, real shifts faster than a range-only Hoeffding bound while keeping the stationary **false-alarm rate `<= delta`** (a Bernstein guarantee). Only bucket boundaries are tested (`O(log width)` splits), and on a detected cut the oldest bucket is dropped and the scan repeats -- so `add` is amortized O(1) and **0 B/op including the cut-scan and the shrink**.
...
The PRNG is a seeded `Uint32` xorshift (`seed` option + getter, never `Math.random`), so the decay -- and therefore the top-k and the witness -- is fully reproducible. Keys are safe integers; on the mutating path (`add` / `addFrom`) a bad key / weight / buffer throws `[lite-adaptive]` typeof-first (a byte-identical no-op), while the `estimate(key)` query never throws -- a bad key reads `NaN` (F12). There is no `merge`: HeavyKeeper decays natively, so it needs no A/B rotation. The witness gates 100% recall of the true top-k above `N/k` vs an exact `Map` oracle (weighted and unit streams) and a bounded ONE-SIDED error -- a reported count is never above the truth and at most `~N/w` below it (HeavyKeeper never overestimates); a frozen-forest and a decay-disabled variant are rejected by the same gate -- see [Testing](#testing).
</details>
...
On `add(now, key)`, an inline two-lane murmur hash (design-parity with lite-sketch's HyperLogLog, never a dep) maps the key to a register and a `rho`; the entry is appended to that register's ring after dropping every dominated tail entry (a newer arrival with `>= rho` outlives an older one, so the older can never be a future window-max). `add` drops expired head entries (`stamp <= now - W`) before appending, so `overflows` counts only IN-WINDOW drops; `count(w?)` is a PURE read -- it skips expired entries, takes each register's live max `rho`, and runs Ertl's improved estimator (`sigma` / `tau`) -- all 0-alloc on the hot path, incl. the windowed eviction.
...
Answer "what's p50 / p99 *over the last `W`*?" in fixed preallocated space at DDSketch accuracy. `SlidingDDSketch` is the recency sibling of `@zakkster/lite-sketch`'s cumulative DDSketch: it keeps a ring of `panes + 1` preallocated DDSketch panes (`panes` is `B`, the user knob, default 32), each covering `W / panes` of the window. Every pane maps a value to the same DDSketch bin -- `key = ceil(log_gamma(value))` with `gamma = (1 + alpha) / (1 - alpha)` -- so a merge across panes is just an element-wise bin sum, and the relative error of any reported quantile stays `<= alpha` (identical mapping, collapse, `strict`, declared `range` and getters to lite-sketch, so a consumer pre-checks a value's indexable range exactly as it does there).
...
const qs = new Float64Array([0.5, 0.99]);   // qs must be a Float64Array (a plain array throws)
const out = new Float64Array(2);
q.quantileInto(qs, out);     // 0 B/call render path: out = [p50, p99]
q.count();                   // the windowed population (0 on empty)
```

`add(now, value)` writes the current pane; when `now` crosses a pane boundary it rotates to the next pane and clears it with a `fill(0)` (0-alloc). A `now` jump of many pane-widths (an idle gap, an epoch-ms clock) clears at most `panes + 1` panes -- the rotation is bounded, never a loop over the skipped span. `quantile` / `quantileInto` / `count` are cold reads that merge the live panes (including the straddling oldest pane) into an **instance-owned preallocated scratch** (never allocated per query); an empty window reads `NaN` with `count() === 0`, never a misleading `0`. A bad query VALUE never throws: `quantile(q)` with `q` outside `[0, 1]` or `NaN` returns `NaN`, and a bad sub-window `w` (`<= 0`, `> W`, `NaN`) returns `NaN` for BOTH `quantile` and `count` (`NaN`, not `0` -- null is not zero). `quantile` returns one boxed double (16 B/call); `quantileInto` is 0 B/call -- use it on a render path. A wrong CONTAINER type (a non-`Float64Array` `qs`/`out`, or `out` too short) is a programming error and still throws.

<details>
<summary>The pane ring, the edge error, and the counters</summary>

The window **over-covers by up to one pane width** `W / panes`: the covered span is `[W, W + W/panes]` -- always the FULL window `W`, over-covered by at most one pane and NEVER under-covered. The ring holds `panes + 1` panes and the straddling oldest pane (the one that spans the `now - W` boundary) is KEPT, never dropped, so `true(W)` is always included; dropping it would under-count (the same one-sided reasoning as `SlidingCountMin`, and exactly the bug the pre-1.7.0 `panes`-only ring had -- it dropped the straddling pane up to one pane width early and under-reported the count). That is the disclosed price of a fixed-memory sliding window over an exact `O(W)` sort; the default 32 panes puts the over-coverage at ~3% of `W`, and it is the caller's knob (more panes -> tighter edge, linearly more memory). Per-pane bin counts are a `Uint32Array` that **saturates at `2^32 - 1`** (never wraps); the merge scratch sums the panes in `Float64`, so a merged window count stays exact well past `2^32`. Each pane collapses its lowest bins independently (the DDSketch `maxBins = 2048` bound), so the merged min-key can differ from a single sketch's -- the accuracy bound is therefore **witnessed**, not assumed. A value whose bin key would exceed `SLD_KEY_MAX` (`1 << 30`) is rejected fail-closed, so the bin index can never overflow.
</details>

`SlidingDDSketch` locks EXPLICIT vs COUNT mode at the first add (a switch throws), and `alpha` / `strict` / `panes` / `W` are validated typeof-first before any allocation. The windowed-quantile witness gates the relative error `<= alpha` vs an exact windowed-sorted-array oracle on 100% of `>= 2000` queries across a `W` x `alpha` sweep, a distribution shift, and a post-burst edge, and asserts the TRUE-window gate: `count() >= true count in (now - W, now]` on 100% of the queries, with quantiles within `alpha` of the covered span `[W, W + W/panes]`; a no-expiry variant (stale out-of-window values counted), a coarse `panes = 2` variant (the edge bound blows past `W / 32`), and a `panes`-pane control (the pre-1.7.0 ring that drops the straddling pane and under-counts) are rejected by the same gate -- see [Testing](#testing). `advance(now)` rotates + clears stale panes with no value added (0 B/op), so an idle window's `quantile` reads `NaN` (and `count()` reads 0) instead of freezing on the last burst (R11 idle-slide).
...
cm.estimate(key);            // COLD; the windowed count for this label (a double; 0 for an unseen VALID key; NaN on a bad key / bad w -- never throws)
...
// getters
windowSize   epsilon   bucketCount   capacity   k   levels   maxCount   mode
```

- **`W`** -- window size; a finite number `> 0` (items in count mode, or the `now`-unit span in explicit mode).
- **`epsilon`** -- relative-error knob in `(0, 1)`; smaller means more buckets and tighter windowed error. The derived bucket-pool capacity `(k+1)*levels + 2` is capped at `2^22` buckets (`EH_CAP_MAX`, ~150 MB at 36 B/bucket); a tiny epsilon (huge `k`) past it throws a tagged `[lite-adaptive]` RangeError BEFORE any allocation instead of aborting the process on a V8 fatal (this rejects an epsilon below ~3e-6 at the default `maxCount`).
- **`options.maxCount`** -- the window population the pool is GUARANTEED to hold, in EITHER mode (the exact ceiling is `k * (2^levels - 1)` >= `maxCount`); a positive integer `<= 2^53-1` (default 2^32), validated typeof-first before allocation (`null` is not a default). It sizes the fixed pool via `levels = max(2, ceil(log2(maxCount/(k+1))) + 2)`. An add whose cascade would pass the top level throws `[lite-adaptive]`. Pass `maxCount: W` in count mode to keep the pre-1.7.0 W-sized pool.
...
- **`count()` / `query()`** -- COLD windowed COUNT estimate; never throw (0 on an empty window). Relative error `<= epsilon`, HARD, on every query.
- **`sum()`** -- COLD windowed SUM estimate; never throws. Absolute error `<= size(oldest straddling bucket) / 2`; relative `<= epsilon` only for count or near-constant values (heavy-tailed / spiky values can exceed it -- levels are sized by POPULATION, not value mass). Use `count()` for a relative bound.
...
| `maxCount` | default `2^32` | guaranteed window population; sizes the pool (exact ceiling `k * (2^levels - 1)`) |
| `levels` | `max(2, ceil(log2(maxCount/(k+1))) + 2)` | number of size-class levels the pool can occupy |
| `capacity` | `(k+1) * levels + 2` | the fixed bucket-pool size (never grows) |
| count error | `~ 1/(2k) <= epsilon` | windowed COUNT relative error, HARD, on every query |
| sum error | `<= size(straddling bucket) / 2` | absolute; relative `<= epsilon` only for count / near-constant values |
...
- **`add(x)`** -- append a finite real `x` (item-indexed; no clock; `|x| <= sqrt(Number.MAX_VALUE) / 2 ~= 6.7e153` since 1.7.0, because the sums are centred on an offset and `|x - c|` must still square finitely), run the ADWIN2 variance-aware cut over the bucket boundaries, and on a change DROP the older sub-window. Returns `true` exactly on the item that detects the change. Fail closed: a non-finite / non-number `x`, or a finite `|x|` whose square would overflow, is a byte-identical no-op that throws `[lite-adaptive]`.
...
estimate(key) -> number        // COLD: max matching cell count (0 for unseen but VALID key; bad key -> NaN, never throws)
...
advanceFrom(buf, i) -> this    // HOT, 0 B/op ZERO-BOX: now = buf[i] (explicit-time)
quantile(q, w?) -> number      // COLD, ONE boxed return (16 B); windowed quantile; NaN on empty / bad q / bad w (never throws)
quantileInto(qs, out) -> number// COLD, 0 B/call; write each qs[j]'s quantile into out; returns the count; THROWS on a non-Float64Array
count(w?) -> number            // COLD; windowed population (0 on empty); NaN on a bad w in (0, W] (never throws)
...
advanceFrom(buf, i) -> this    // HOT, 0 B/op ZERO-BOX: now = buf[i] (explicit-time)
estimate(key, w?) -> number    // COLD, O(d*(B+1)); sum-then-min over live panes; a DOUBLE; NEVER throws (0 for unseen VALID key; NaN on bad key / bad w); w in (0, W]; a windowed count >= 2^31 boxes one 16 B HeapNumber (F6)
...
- **`d` / `w` / `k`** -- hash rows (`~4-8`), cells per row, and the top-k size. `HeavyKeeper.withAccuracy(k, targetError)` derives `d` / `w` from a target relative error. A bad `d` / `w` / `k` / `seed` / `b` / option throws `[lite-adaptive]` typeof-first, before any allocation. Memory caps are checked BEFORE allocation and throw a tagged RangeError naming the cap: `d*w <= 2^27` cells (`HK_CELLS_CAP`, ~1 GB at 8 B/cell), `w <= 2^30` (`HK_W_MAX`), `k <= 2^24` (`HK_K_MAX`, ~256 MB at 16 B/slot) -- an oversized table used to abort the process on a V8 fatal.
...
- **`add(key, weight = 1)`** -- `key` a SAFE INTEGER; `weight` an integer in `[1, 2^32-1]` (rank by count when 1, or by total time / bytes / any additive weight). Fail closed: a non-safe-integer key, or a weight that is not an integer in `[1, 2^32-1]`, throws `[lite-adaptive] HeavyKeeper weight must be an integer in [1, 4294967295], got <w>` (a byte-identical no-op). A single over-range weight throws (in 1.6.0 it wrapped a `Uint32` cell so `estimate` read `0` while `topK` reported `2^32`); an ACCUMULATED cell SATURATES at `2^32-1` (the two rules are distinct -- F10). This is parity with `SlidingCountMin`'s `[1, 2^32-1]` count domain.
...
| `w` | cells / row | more cells -> fewer collisions -> a smaller underestimate (at most `~ N/w` below the truth; never above) |
...
- **`count(w?)`** -- the windowed distinct estimate; `w` an optional sub-window in `(0, W]` (outside that range returns `NaN`, never throws -- F12; `w` omitted queries the full window W, an empty window reads 0). `standardError` is `1.04 / sqrt(m)`, guaranteed while `degraded === false`.
...
- **`SlidingDDSketch(W, { alpha, strict, panes, range })`** -- `W` the window (items in count mode, or the `now`-unit span in explicit mode), a finite number `> 0`; `alpha` the relative-error knob in `(0, 1)` (default `0.01`); `panes` the pane count `B`, an integer `>= 2` (default 32; the ring holds `panes + 1` and over-covers by up to `W / panes`); `range` a declared `[rmin, rmax]` band (finite `0 < rmin < rmax`, both inside the alpha indexable band, needing `<= 2048` bins) that **derives** strict with a fixed bin offset (lite-sketch DDSketch parity). A declared `range` with `strict: false` is a contradiction and throws. A bad `W` / `alpha` / `strict` / `panes` / `range` / option throws `[lite-adaptive]` typeof-first, before any allocation. `alpha` is an option here (the `(W, options)` convention), where lite-sketch's cumulative `DDSketch` takes it as the leading positional -- the mapping and accuracy contract are otherwise identical.
...
- **`quantile(q, w?)` / `quantileInto(qs, out)`** -- `q` in `[0, 1]` (outside / `NaN` returns `NaN`, never throws); an empty window returns `NaN` (never `0`). `w` an optional sub-window in `(0, W]` (outside / `NaN` returns `NaN` for both `quantile` and `count` -- `NaN`, not `0`; null is not zero). `quantileInto` is the 0-B/call render path -- it writes each `qs[j]`'s quantile into the caller's `out` and returns the count written; a wrong CONTAINER type (a non-`Float64Array` `qs`/`out`, or `out.length < qs.length`) is a programming error and THROWS. `quantile` returns one boxed double (16 B/call). Both merge the live panes into an instance-owned scratch (cold, 0-alloc).
- **`minIndexable` / `maxIndexable` / `rangeMin` / `rangeMax` / `collapsed` / `strict`** -- the DDSketch accuracy getters, identical in name and meaning to lite-sketch. `[minIndexable, maxIndexable]` is the value range the sketch can represent within `alpha` (ALPHA-ONLY: identical across non-strict / strict / range). `collapsed` reports whether any pane collapsed its lowest bins (always `false` in strict mode -- strict never collapses). `strict` (default `false`) fails closed instead of collapsing: WITHOUT a `range` it is span-based (a pane throws only when its occupied key span would exceed 2048 bins, otherwise it re-anchors losslessly); WITH a declared `range` it is fixed to `[rmin, rmax]`. `rangeMin` / `rangeMax` are the declared band (NaN when undeclared). This is the F2 parity fix -- the getters were already lite-sketch-shaped; the gap was the missing `range` option that derives strict, now closed. A consumer pre-checks a value against `[minIndexable, maxIndexable]` exactly as it does for lite-sketch `DDSketch`.
...
| `panes` | default 32, `>= 2` | pane count `B` (the user knob); the ring holds `panes + 1` and over-covers by up to one pane width `W / panes` (~3% at 32) |
| `maxBins` | 2048 (`SLD_MAX_BINS`) | per-pane bin bound (matches lite-sketch's default); lowest bins collapse past it |
| space | `(panes + 1) * maxBins * 4 B` + scratch | `Uint32` bin counts per pane over `B + 1` panes (saturating at `2^32-1`) + a `Float64` merge scratch |

- **`DecayedReservoir(k, halfLife, { seed })`** -- `k` the sample size, a positive integer `<= 2^24` (`DR_K_MAX`, ~256 MB at 16 B/slot -- a bigger `k` throws a tagged RangeError BEFORE allocation instead of a lazy multi-GB over-commit); `halfLife` a finite number `> 0` (`lambda = ln2 / halfLife`); `seed` a `Uint32` for the A-Res PRNG (default `0x9e3779b1`; `seed = 0` valid, guarded `!== undefined`). A bad `k` / `halfLife` / `seed` / option throws `[lite-adaptive]` typeof-first, before any allocation. There is no `advance()` -- a reservoir is a sample, not a hard window (like `ForwardDecay`).
...
Across the roster, the hot ops (`add` / `addFrom` / `advance` / `advanceFrom`) are **0 B/op** including every amortized reshaping (merge / expire, pane rotate + `fill(0)`, forest sift, landmark rebase). The cold readers split by whether they RETURN a fractional / large double:

| Member | Hot ops (0 B/op) | Cold reader | Alloc |
|--------|------------------|-------------|-------|
| ExponentialHistogram | `add` / `addFrom` / `advance` / `advanceFrom` | `count()` / `query()` | 0 B/call |
| | | `sum()` | 0 steady (boxes 16 B only in the early JIT tier) |
| ADWIN | `add` / `addFrom` (-> boolean) | `mean` / `variance` / `width` | 0 B/call |
| ForwardDecay | `add` / `addFrom` | `count` / `sum` / `mean` / `rate` | 0 B/call |
| HeavyKeeper | `add` / `addFrom` | `forEach` / `topKInto` | **0 B/call** (the render path) |
| | | `estimate` | 0 steady (early-JIT-tier box) |
| | | `topK()` | allocates (cold convenience) |
| SlidingHyperLogLog | `add` / `addFrom` / `advance` / `advanceFrom` | `count(w?)` | 0 B/call |
| DriftDetector | `add` / `addFrom` (-> boolean) | `mean` / `statistic` | 0 B/call |
| SlidingDDSketch | `add` / `addFrom` / `advance` / `advanceFrom` | `quantileInto(qs, out)` / `count(w?)` | **0 B/call** (the render path) |
| | | `quantile(q, w?)` | 16 B/call (one boxed double -- use `quantileInto`) |
| SlidingCountMin | `add` / `addFrom` / `advance` / `advanceFrom` | `estimate(key, w?)` | 0 B/call for a count < 2^31; **16 B/call** when the windowed count >= 2^31 (`estimateInto` in 1.8.0) |
| DecayedReservoir | `add` / `addFrom` | `sampleInto(buf)` / `forEach` | **0 B/call** (the render path) |

One rule (F6): a query that RETURNS a fractional or large double boxes one 16 B HeapNumber per call at a non-inlined call site; the `Into` / `forEach` readers are the 0-alloc render path.

Gated quality numbers (`npm run verify`):

- **Recency**: windowed COUNT relative error `<= epsilon` on EVERY query across `W in {64, 1000, 65536} x epsilon in {0.5, 0.1, 0.01}`, plus a shifting-rate stream and a dense 10 kHz / 70 s stream (count() never NaN under the default maxCount); a broken EH (no straddle correction) is REJECTED by the same gate. The SUM bound `|err| <= size(straddling bucket)/2` holds on 100% of queries over uniform / heavy-tail / spike streams (the old `sum <= epsilon` claim fails on skew).
...
- **Throughput + steady-state allocation**: the perf gate (`test:perf`) is flat with a must-allocate control that trips it; `AllocProbe` reports steady-state **B/op** (the min over >= 4 windows, both semi-space flags pinned) across the matrix gates (`test:perf:matrix`, `gates:red`): the key x clock matrix (each `addFrom` x {performance.now, epoch-ms} x keys {2^30, 2^31, 2^32-1, -2^31, 2^53-1}, fresh + warmed), the query lanes (`quantileInto` / `count` / `estimate` / `sum` / `forEach` / `sampleInto`), a +1e12 jump-timing gate (SlidingCountMin / ForwardDecay / DecayedReservoir), the true-window SlidingDDSketch gate, and the `test/differential` parity suites (HeavyKeeper / SlidingDDSketch / SlidingHyperLogLog bit-identical to the 1.6.0 vectors).
...
- **A query that returns a fractional or large double boxes one 16 B HeapNumber per call** at a non-inlined call site; the `Into` / `forEach` readers (`quantileInto`, `topKInto`, `sampleInto`, `forEach`) are the 0-alloc render path. `SlidingCountMin.estimate` boxes only when the windowed count is >= 2^31 (`estimateInto` lands in 1.8.0); `SlidingDDSketch.quantile` boxes one double (use `quantileInto`); `sum()` / `HeavyKeeper.estimate` box only in the early JIT tier (steady 0). Integer counts are Smis, 0 B/call (F6).
...
- **One query contract across the package (F12).** A bad query VALUE returns `NaN` and never throws; a wrong container TYPE throws. A bad quantile `q`, an out-of-range sub-window `w`, an out-of-domain key -- all read `NaN` (`SlidingDDSketch.quantile`/`count`, `SlidingHyperLogLog.count`, `SlidingCountMin.estimate`, `HeavyKeeper.estimate`), because a render path must be able to poll freely without a try/catch. `NaN` is deliberately NOT `0`: an unrepresentable window is not an under-count, and an invalid key was never "seen 0 times". Only an UNSEEN but VALID key on the count sketches reads a legitimate `0`. A wrong container TYPE stays a programming error: an Into reader handed a non-`Float64Array` (or one too short) still throws.
- **One option door with a did-you-mean hint (F13, R6).** Every constructor (all 9) and both `withAccuracy` factories validate `options` through a single shared door: `undefined` is OK; `null`, a non-object, an `Array`, or a typed array is a `TypeError`; an unknown OWN key is a `RangeError` with a Levenshtein `<= 2` suggestion (`{sede: 1}` -> `did you mean "seed"?`). The known-key sets are null-proto, so inherited keys like `toString` / `constructor` can never sneak through as options (they could on six classes before 1.7.0). An unknown key is an error with a hint, never a silent ignore -- `null` is not zero.
...
- **SlidingDDSketch is a pane ring, not an EH-of-sketches, and that is a zero-GC decision.** A windowed quantile wants to expire old values without an `O(W)` sort. The exact-edge option -- a DGIM / exponential-histogram *of DDSketches* -- has to *merge* sketches on `add`, which allocates; and Arasu-Manku true windowed quantiles keep unbounded per-item state. Both fail the 0-B/op contract. A fixed ring of `panes + 1` preallocated sketches keeps `add` allocation-free (rotate + `fill(0)`, bounded even on a huge `now` jump) and merges only at query, into an instance-owned scratch. The cost is an honest, disclosed edge -- the window over-covers by up to one pane width `W / panes`, covering `[W, W + W/panes]`, keeping the straddling oldest pane so it never under-counts (the same one-sided bound as `SlidingCountMin`) -- and because each pane collapses its lowest bins independently, the merged accuracy is *witnessed* against an exact oracle rather than assumed (see ADR 0008).
...
- `npm run witness` -- the recency witness: the windowed error vs the `epsilon` bound (EH) + the change-response gates (ADWIN) + the exact-aggregate gate `|fd - oracle| / |oracle| <= 1e-9` (ForwardDecay) + the top-k recall / marquee (HeavyKeeper) + the windowed-distinct error vs a `Set` oracle (SlidingHyperLogLog) + the detection-latency / false-alarm gates (DriftDetector) + the windowed-quantile error `<= alpha` vs a sorted-array oracle with the TRUE-window gate `count() >= true count in (now - W, now]` on 100% and quantiles within `alpha` of the covered span `[W, W + W/panes]` (SlidingDDSketch) + the one-sided windowed-frequency bound `true(W) <= est <= true(W + W/B) + epsilon * N` vs an exact windowed oracle (SlidingCountMin) + the recency-sample inclusion rate by age matching `exp(-lambda * age)` over many seeds (DecayedReservoir), each with rejected negative controls (for DecayedReservoir: a no-decay uniform reservoir and a no-forest "keep the first k" reservoir).
- `npm run torture` -- the 0 B/op leak + GC-profiler gate on `add` including the merge / expire reshaping (`node --expose-gc`).
- `npm run test:perf` -- the flat-throughput perf gate (**30 tests**) + a must-allocate control, both semi-space flags pinned at `maxScavenges: 0`.
- `npm run test:perf:matrix` -- the allocation matrix (**58 tests**): the `AllocProbe` steady-state B/op probe, the key x clock x count fresh/warmed matrix (`AllocMatrix`), the query lanes, and the +1e12 jump-timing gate (`JumpTiming`).
- `npm run gates:red` -- the strict (`LITE_GATES_STRICT=1`) run of `test:perf` + `test:perf:matrix` + `witness` together; the has-teeth revert-check for the allocation and true-window gates.
- `npm run test:types` -- the ambient type-surface compile check.
- `npm run verify` -- `npm test` + `test:types` + `torture` + `witness` + `test:perf` + `demo:check` (the release gate).

`npm test` runs **439 tests** across the `node:test` behavioral suites and the `test/differential` parity suites (HeavyKeeper / SlidingDDSketch / SlidingHyperLogLog replayed bit-identical to their pinned 1.6.0 vectors).
...
- **Not a cumulative or an exact quantile.** `SlidingDDSketch` answers quantiles over the last `W` and forgets older values; for a whole-stream quantile that never forgets, use `@zakkster/lite-sketch`'s DDSketch. It *estimates* within `alpha` relative error (not an exact percentile -- that needs an `O(W)` sort you can hold), and its window over-covers by up to one pane width `W / panes`, covering `[W, W + W/panes]` -- the full window, never under-covered (the disclosed edge). It is the third additive post-1.0 member (1.3.0), landing without breaking the frozen core.
