// @zakkster/lite-adaptive -- demo static file server (repo-only dev artifact, NEVER shipped).
//
//   npm run demo:serve            (then open http://localhost:8040/)
//   node demo/serve.mjs [port]
//
// A zero-dependency Node http server (node:http + node:fs only). Its web root is the LiteAdaptive
// REPO ROOT, so the page at /demo/index.html and its `../Adaptive.js` + `./kernels.mjs` imports all
// resolve from one origin. There is no dynamic route -- lite-adaptive's demo drives the REAL shipped
...
const ROOT = dirname(DEMO_DIR); // repo root -- so `../Adaptive.js` from /demo resolves to /Adaptive.js
...
export async function handle(req, res) {
    ...
    // (../Adaptive.js, ./kernels.mjs) would resolve wrong. Redirecting makes the browser re-request
...
export function start(port) {
    ...
    server.listen(port, () => {
        process.stdout.write('lite-adaptive demo server on http://localhost:' + port + '/\n');
